...
class BaseTestObject(TestCase):
    ...
    def assert_mail_empty(self):
        ...
        self.assertEqual(
            len(mail.outbox),
            0,
            f"Mail queue should have been empty. Current contents: {self._mail_to_str()}",
        )
    ...
    def validate_tool_post(self, url, data):
        return self.client.post(
            f"{url}{self.validation_url_extension}", data, format="json"
        )

    def validate_tool_put(self, id, data):
        return self.client.put(
            f"/{id}/{self.validation_url_extension}",
            data,
            format="json",
            HTTP_ACCEPT="application/json",
        )
