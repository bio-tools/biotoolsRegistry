...
class ArrayTester:
    ...
    def create_values_for_constraints(value, is_valid: bool, constraint_dict: dict, value_dict: dict):
        ...
        if MIN_ITEMS in constraint_dict:
            ...
            ArrayTester.create_minItems_test_values(value, minItems_value, value_dict, is_valid)
        if MAX_ITEMS in constraint_dict:
            maxItems_value = constraint_dict[MAX_ITEMS]
            ArrayTester.create_maxItems_test_values(value, maxItems_value, value_dict, is_valid)
    ...
    # MIN ITEMS --------------------------------------------------------------------------------------------------------
    @staticmethod
    def create_minItems_test_values(value, min_items: int, value_dict: dict, is_valid: bool):
    ...
    # MAX ITEMS --------------------------------------------------------------------------------------------------------
    @staticmethod
    def create_maxItems_test_values(value, max_items: int, value_dict: dict, is_valid: bool):
