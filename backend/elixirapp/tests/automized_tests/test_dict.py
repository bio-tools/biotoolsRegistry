...
class SchemaParser:
    ...
    def build_dict(tool_properties):
        level_one_properties = tool_properties.keys()  # extract names of all 1st level properties
        for prop in level_one_properties:
            prop_def = tool_properties[prop]  # get property definition

            if SchemaParser.has_type_attribute(prop_def):
                SchemaParser.handle_property(prop_def, prop,  [])  # parse the property
            else:
    ...
    @staticmethod
    def has_type_attribute(property: object):
        return "type" in property

    @staticmethod
    def handle_property(property: object, prop_name: str, path_to_prop: list):
        prop_type = property["type"]
        path_to_prop.append(prop_name)

        if prop_type == TYPE_STRING_STR:
            SchemaParser.parse_string(property, path_to_prop)
        elif prop_type == TYPE_ARRAY_STR:
            SchemaParser.parse_array(property, path_to_prop)  # TODO extend path
        elif prop_type == TYPE_OBJECT_STR:
            SchemaParser.parse_object(property, path_to_prop)
        ...
        if "type" in array_prop and array_prop["type"] in [TYPE_STRING_STR]:
            ...
            if "type" in items:
                if items["type"] == TYPE_STRING_STR:
                    SchemaParser.handle_property(array_prop["items"], None, path)
                elif items["type"] == TYPE_OBJECT_STR:
                    for prop in items["properties"]:
                        path.append(prop)
                        SchemaParser.parse_array(items["properties"][prop], path)
                        path.remove(prop)
    ...
    def parse_object(object_prop, path):
        for prop_key, prop_val in object_prop["properties"].items():
            SchemaParser.handle_property(prop_val, prop_key)

    @staticmethod
    def parse_string(string_prop, path):
        SchemaParser.create_dict_entry(string_prop, path)
