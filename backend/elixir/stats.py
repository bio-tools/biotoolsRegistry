...
class StatsInfo():
	...
	def topContributors(self, limit, upperDateLimit=datetime.today()):
		...
		emailsList = filter(lambda data: data['email'] != None and data['email'] != '' and '@' in data['email'], emailsList) # discard empty emails
		emailsList = map(lambda data: data['email'].split(".").pop(), emailsList) # extract domains
		...
		emailsList = map(lambda data: { 'domain': data[0], 'count': data[1]}, emailsList) # format data
		return emailsList
	...
	def accessibilityAnnotationCount(self, upperDateLimit=datetime.today()):
		return {'accessibilityAnnotationCount': Accessibility.objects.filter(resource__visibility=1, additionDate__lt=upperDateLimit).count()}
	...
	def topSearchStats(self, limit):
		return logging.search_log_json_data()[:limit]
