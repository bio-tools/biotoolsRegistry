...
class StatsInfo:
    ...
    def statsDataForLast(self, days):
        return StatsData.objects.filter(
    ...
    def totalEntriesForLast(self, months):
        ...
        for index, stat in enumerate(stats):
            ...
            if index < len(stats) - 1:
                ...
                if (
                    stat.date.month != lastAddedDate.month
                    and stat.date.month != stats[index + 1].date.month
                ):
    ...
    def generateStatsEntry(self, stat):
        return {
            "date": stat.date,
            "entriesCount": stat.totalEntries,
            "creditAffiliationCount": stat.creditAffiliationCount,
    ...
    def totalEntries(self, upperDateLimit=datetime.today()):
        return Resource.objects.filter(
            visibility=1, additionDate__lt=upperDateLimit
        ).count()
    ...
    def edamAnnotationCount(self, upperDateLimit=datetime.today()):
        edamAnnotations = 0
        edamAnnotations += Operation.objects.filter(
            function__resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        edamAnnotations += Topic.objects.filter(
            resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        edamAnnotations += Format.objects.filter(
            input__function__resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        edamAnnotations += Format.objects.filter(
            output__function__resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        edamAnnotations += Data.objects.filter(
            input__function__resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        edamAnnotations += Data.objects.filter(
            output__function__resource__visibility=1, additionDate__lt=upperDateLimit
    ...
    def formatAnnotationsCount(self, upperDateLimit=datetime.today()):
        formatAnnotations = 0
        formatAnnotations += Format.objects.filter(
            input__function__resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        formatAnnotations += Format.objects.filter(
            output__function__resource__visibility=1, additionDate__lt=upperDateLimit
    ...
    def dataTypeAnnotationsCount(self, upperDateLimit=datetime.today()):
        dataAnnotations = 0
        dataAnnotations += Data.objects.filter(
            input__function__resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        dataAnnotations += Data.objects.filter(
            output__function__resource__visibility=1, additionDate__lt=upperDateLimit
    ...
    def topTopics(self, limit, upperDateLimit=datetime.today()):
        topicCountList = (
            Topic.objects.exclude(term="N/A")
            .filter(resource__visibility=1, additionDate__lt=upperDateLimit)
            .values("term")
            .annotate(count=Count("term"))
        )
        topicCountList = sorted(
            topicCountList, key=lambda topic: topic["count"], reverse=True
        )[:limit]
        topicCountData = [
            {"topic": topic["term"], "count": topic["count"]}
            for topic in topicCountList
        ]
    ...
    def topFunctions(self, limit, upperDateLimit=datetime.today()):
        functionCountList = (
            Operation.objects.exclude(term="N/A")
            .filter(function__resource__visibility=1, additionDate__lt=upperDateLimit)
            .values("term")
            .annotate(count=Count("term"))
        )
        functionCountList = sorted(
            functionCountList, key=lambda function: function["count"], reverse=True
        )[:limit]
        topicCountData = [
            {"function": function["term"], "count": function["count"]}
            for function in functionCountList
        ]
        topicCountData = [
            data for data in topicCountData if data["function"] != "Operation"
        ]
        return topicCountData
    ...
    def topDataTypes(self, limit, upperDateLimit=datetime.today()):
        ...
        dataTypeCountList = sorted(
            dataTypeCountList, key=lambda dataType: dataType["count"], reverse=True
        )[:limit]
        dataTypeCountData = [
            {"dataType": dataType["term"], "count": dataType["count"]}
            for dataType in dataTypeCountList
        ]
        dataTypeCountData = [
            data for data in dataTypeCountData if data["dataType"] != "Data"
        ]
    ...
    def topDataFormats(self, limit, upperDateLimit=datetime.today()):
        ...
        dataFormatCountList = sorted(
            dataFormatCountList,
            key=lambda dataFormat: dataFormat["count"],
            reverse=True,
        )[:limit]
        dataFormatCountData = [
            {"dataFormat": dataFormat["term"], "count": dataFormat["count"]}
            for dataFormat in dataFormatCountList
        ]
        dataFormatCountData = [
            data for data in dataFormatCountData if data["dataFormat"] != "Format"
        ]
    ...
    def topContributors(self, limit, upperDateLimit=datetime.today()):
        ...
        emailsList = [
            data
            for data in emailsList
            if data["email"] is not None
            and data["email"] != ""
            and "@" in data["email"]
        ]  # discard empty emails
        emailsList = [
            data["email"].split(".").pop() for data in emailsList
        ]  # extract domains
        ...
        emailsList = [
            {"domain": data[0], "count": data[1]} for data in emailsList
        ]  # format data
        return emailsList
    ...
    def nameAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "nameAnnotationCount": Resource.objects.filter(
                visibility=1, additionDate__lt=upperDateLimit
            )
            .exclude(name="")
            .count()
    ...
    def descriptionAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "descriptionAnnotationCount": Resource.objects.filter(
                visibility=1, additionDate__lt=upperDateLimit
            )
            .exclude(description="")
            .count()
    ...
    def homepageAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "homepageAnnotationCount": Resource.objects.filter(
                visibility=1, additionDate__lt=upperDateLimit
            )
            .exclude(homepage="")
            .count()
    ...
    def toolTypeAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "toolTypeAnnotationCount": ToolType.objects.filter(
                resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def uniqueIDAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "uniqueIDAnnotationCount": Resource.objects.filter(
                visibility=1, additionDate__lt=upperDateLimit
            )
            .exclude(biotoolsID="")
            .count()
    ...
    def topicAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "topicAnnotationCount": Topic.objects.filter(
                resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def publicationAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "publicationAnnotationCount": Publication.objects.filter(
                resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def contactAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "contactAnnotationCount": Contact.objects.filter(
                resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def operationAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "operationAnnotationCount": Operation.objects.filter(
                function__resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def documentationAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "documentationAnnotationCount": Documentation.objects.filter(
                resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def operatingSystemAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "operatingSystemAnnotationCount": OperatingSystem.objects.filter(
                resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def inputOutputAnnotationCount(self, upperDateLimit=datetime.today()):
        count = 0
        count += Input.objects.filter(
            function__resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        count += Output.objects.filter(
            function__resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
    ...
    def codeAvailabilityAnnotationCount(self, upperDateLimit=datetime.today()):
        count = 0
        count += Language.objects.filter(
            resource__visibility=1, additionDate__lt=upperDateLimit
        ).count()
        count += (
            Resource.objects.filter(visibility=1, additionDate__lt=upperDateLimit)
    ...
    def accessibilityAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "accessibilityAnnotationCount": Resource.objects.filter(
                visibility=1, additionDate__lt=upperDateLimit
            )
            .exclude(accessibility__isnull=True)
    ...
    def communityAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "communityAnnotationCount": Link.objects.filter(
                resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def downloadsAnnotationCount(self, upperDateLimit=datetime.today()):
        return {
            "downloadsAnnotationCount": Download.objects.filter(
                resource__visibility=1, additionDate__lt=upperDateLimit
            ).count()
        }
    ...
    def userGrowthByMonth(self):
        ...
        for idx, item in enumerate(qs):
            user_data = {}
            user_data["date"] = datetime(
                year=int(item["year"]), month=int(item["month"]), day=1
            ).strftime("%Y-%m-%dT%H:%M:%SZ")
            user_data["newUsersCount"] = item["count_items"]
            if idx == 0:
                user_data["totalUsersCount"] = item["count_items"]
            else:
                user_data["totalUsersCount"] = (
                    item["count_items"] + user_stats[idx - 1]["totalUsersCount"]
                )
