...
def handle_ecosystem_error(
    logger, the_exception, bt=None, log_exception=False, post_issue=False
):
...
def ecosytem_handle_tool_operations(biotools_user, tool, operation):
    ...
    try:
        ...
        if ecosystem_settings.ECO_LOG_ISSUE and logger:
            logger.info(
                "Success! Tool {} PR: {} for tool with id: {} by bio.tools user: {}".format(
                    operation, get_pr_link(pr.number), bt.tool_id, bt.username
                )
            )
