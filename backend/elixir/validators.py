...
def IsURLFTPValidator(value):
    ...
    p = re.compile(
        r"^(https?|s?ftp):\/\/[^\s\/$.?#].[^\s]*$", re.IGNORECASE | re.UNICODE
    )
    if not p.search(value):
        raise serializers.ValidationError(message)
    return value
...
def IsDOIValidator(value):
    ...
    matchDOI = re.compile(
        r"^10\.[0-9]{4,9}\/[-\.\[\]<>_;\(\)\/:a-zA-Z0-9]+$", re.IGNORECASE
    )
    matchNone = re.compile(r"^None$", re.IGNORECASE)
...
class OntologyValidator:
    def find_term_or_uri_in_node(self, node, name_or_uri):
        if len(name_or_uri) > 0:
            ...
            if (
                node["text"].lower() == name_or_uri.lower()
                or node["data"]["uri"].lower() == name_or_uri.lower()
            ):
                ...
                if (
                    node["text"].lower() != name_or_uri.lower()
                    and self.term is not None
                ):
                    if self.term.lower() in e_s:
                        return {
                            "data": {"uri": node["data"]["uri"]},
                            "text": e_s[self.term.lower()],
                            "is_synonym": "True",
                        }
                    elif self.term.lower() in n_s:
                        return {
                            "data": {"uri": node["data"]["uri"]},
                            "text": n_s[self.term.lower()],
                            "is_synonym": "True",
                        }
                ...
                return {
                    "data": {"uri": node["data"]["uri"]},
                    "text": e_s[name_or_uri.lower()],
                    "is_synonym": "True",
                }
            elif name_or_uri.lower() in n_s:
                return {
                    "data": {"uri": node["data"]["uri"]},
                    "text": n_s[name_or_uri.lower()],
                    "is_synonym": "True",
                }
            else:
    ...
    def check_if_term_or_uri_in_ontology(self, check_term):
        ...
        if found is not None:
            ...
            if (
                found["data"]["uri"]
                .lower()
                .find(self.ontology.lower().replace("edam_", ""))
                >= 0
            ):
    ...
    def __call__(self, uri, term):
        ...
        if uri and len(uri) > 0:
            ...
            if found["status"] == "ok":
                ...
                if (
                    term
                    and term.lower().strip() != found["data"]["text"].lower()
                    and found.get("is_synonym") != "True"
                ):
                    message = (
                        "The term does not match the URI: " + term + ", " + uri + "."
                    )
                    raise serializers.ValidationError(message)
...
def is_blacklisted_url_validator(url):
    for blacklisted_url in settings.BLACKLISTED_DOMAINS_LIST:
        if blacklisted_url.lower() in url.lower():
            raise serializers.ValidationError(
                "This URL domain is blacklisted. Contact help@bio.tools for more details."
            )
