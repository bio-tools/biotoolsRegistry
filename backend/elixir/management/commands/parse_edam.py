...
def flatten(lst, node, prev):
    ...
    if prev == "":
        ...
        tmp["path"]["key"] = (
            node["data"]["uri"].lower().replace("http://edamontology.org/", "")
        )
    else:
        ...
        if node.hasChildNodes() and el["remove"] == 0:
            if node.attributes:
                ...
                for n2 in node.childNodes:
                    ...
                    if (
                        n2.nodeName == "oboInOwl:replacedBy"
                        and n2.attributes["rdf:resource"].value
                    ):
                        attrs["replaced_by"].append(n2.attributes["rdf:resource"].value)
                    if (
                        n2.nodeName == "oboInOwl:consider"
                        and n2.attributes["rdf:resource"].value
                    ):
...
def minify_flat_data(flat_data):
    ...
    for e in flat_data:
        ...
        n["ch"] = json.loads(
            json.dumps(e["children"])
            .replace('"name":', '"n":')
            .replace('"uri":', '"u":')
        )
        n["p"] = json.loads(
            json.dumps(e["path"]).replace('"name":', '"n":').replace('"key":', '"k":')
        )
...
def minify_index_data(index_data):
    ...
    for k in list(index_data.keys()):
        ...
        new_index_data[k]["ns"] = index_data[k]["narrow_synonyms"]
        new_index_data[k]["ch"] = json.loads(
            json.dumps(index_data[k]["children"])
            .replace('"name":', '"n":')
            .replace('"uri":', '"u":')
        )
        new_index_data[k]["p"] = json.loads(
            json.dumps(index_data[k]["path"])
            .replace('"name":', '"n":')
            .replace('"key":', '"k":')
        )
...
def ontology_save(name, data, path_edam_json, version):
    ...
    # regular data + min data
    with open(
        path_edam_json + "/" + version + "/" + name + "_" + version + ".json", "w"
    ) as outfile:
        json.dump(data, outfile)
    with open(
        path_edam_json + "/" + version + "/min_" + name + "_" + version + ".json", "w"
    ) as outfile:
        json.dump(min_data, outfile)

    # flat data + min flat data
    with open(
        path_edam_json + "/" + version + "/flat_" + name + "_" + version + ".json", "w"
    ) as outfile:
        json.dump(flat_data, outfile)
    with open(
        path_edam_json + "/" + version + "/min_flat_" + name + "_" + version + ".json",
        "w",
    ) as outfile:
        json.dump(min_flat_data, outfile)
    ...
    # index data + min index data
    with open(
        path_edam_json + "/" + version + "/index_" + name + "_" + version + ".json", "w"
    ) as outfile:
        json.dump(index_data, outfile)
    with open(
        path_edam_json + "/" + version + "/min_index_" + name + "_" + version + ".json",
        "w",
    ) as outfile:
        json.dump(min_index_data, outfile)
    ...
    if len(query) == 0:
        o = Ontology(
            name="index_" + name + "_" + version, data=index_data_structure_json
        )
        o.save()
    # updating records
    elif len(query) == 1:
    ...
    if len(query) == 0:
        o = Ontology(
            name="min_" + name + "_" + version, data=min_ontology_structure_json
        )
        o.save()
    # updating records
    elif len(query) == 1:
    ...
    if len(query) == 0:
        o = Ontology(
            name="min_flat_" + name + "_" + version, data=min_flat_data_structure_json
        )
        o.save()
    # updating records
    elif len(query) == 1:
    ...
    if len(query) == 0:
        o = Ontology(
            name="min_index_" + name + "_" + version, data=min_index_data_structure_json
        )
        o.save()
    # updating records
    elif len(query) == 1:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
            ...
            if marker is not None:
                if "http://edamontology.org/" in marker:
                    ...
                    for n in el.childNodes:
                        if (
                            n.nodeName == "rdfs:subClassOf"
                            and n.attributes
                            and not obsolete
                        ):
                            if (
                                n.attributes["rdf:resource"].value
        ...
        obsolete_tree = {
            "text": "Deprecated",
            "data": {"uri": "http://www.w3.org/2002/07/owl#DeprecatedClass"},
            "narrow_synonyms": [],
            "exact_synonyms": [],
            "children": [],
        }
        ...
        topic_tree = {
            "text": "Topic",
            "data": {"uri": "http://edamontology.org/topic_0003"},
