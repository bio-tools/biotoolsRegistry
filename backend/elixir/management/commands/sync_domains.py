...
def process_domain(domain_object):
    ...
    for c in domain_data["collection"]:
        collection_objects = CollectionID.objects.filter(name=c).select_related(
            "resource"
        )
        ...
        resource_list.extend(
            [
                OrderedDict(
                    [("name", c.resource.name), ("biotoolsID", c.resource.biotoolsID)]
                )
                for c in collection_objects
                if c.resource.visibility == 1
            ]
        )
            ...
            try:
                ...
                if domain_serializer.is_valid():
                    ...
                    updated_domains.append(
                        f"{settings.URL_FRONT}/t?domain={domain_serializer.validated_data['name']}"
                    )
