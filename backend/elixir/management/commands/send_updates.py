...
def get_new_tools(day: datetime.date):
    res = Resource.objects.filter(visibility=1, additionDate__date=day).order_by(
        "biotoolsID"
    )
...
def get_updated_tools(day: datetime.date):
    res = Resource.objects.filter(visibility=1, lastUpdate__date=day).order_by(
        "biotoolsID"
    )
    return format_tool_results(res)
...
def get_deleted_tools(day: datetime.date):
    res = Resource.objects.filter(visibility=0, lastUpdate__date=day).order_by(
        "biotoolsID"
    )
    l = []
    for r in res:
