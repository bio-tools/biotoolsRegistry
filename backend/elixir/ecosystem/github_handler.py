...
class GithubToolHandler:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise EcosystemLoginException(
                self.__username, ecosystem_settings.ECO_GITHUB_REPO_NAME, str(bce)
            )
            ...
            self.__repo = None
            raise EcosystemLoginException(
                self.__username, ecosystem_settings.ECO_GITHUB_REPO_NAME, str(uoe)
            )
    ...
    def __get_branch_name(self, username, tool_id, delete=False):
        return "{u}__{t}{d}".format(
            u=username, t=tool_id, d=self.DELETED_BRANCH_SUFFIX if delete else ""
        )
    ...
    def __get_filepath(self, tool_id):
        return os.path.join(
            self.ROOT_GITHUB_FOLDER, tool_id, self.__get_filename(tool_id)
        )
    ...
    def __get_sha(self, file_path, branch_name):
        ...
        try:
            ...
            raise EcosystemException(
                "Could not update tool, either filepath or branch do not exist."
                + str(e),
                branch=branch_name,
                filepath=file_path,
                sha=None,
    ...
    def __create_branch(self, branch_name, from_branch=None):
        ...
        try:
            ...
            self.__repo.create_git_ref(
                ref="refs/heads/" + branch_name, sha=mb.commit.sha
            )
        except GithubException as e:
            raise EcosystemException(
    ...
    def __create_data(
        self, branch_name, new_file_path, custom_message=None, custom_content=None
    ):
        """
        ...
        if custom_message:
            ...
            commit_message = self.__create_commit_message(
                self.CREATE, self.__bt.username, self.__bt.tool_id
            )
    ...
    def __update_data(self, update_branch_name, update_file_path, file_sha):
        ...
        """
        commit_message = self.__create_commit_message(
            self.UPDATE, self.__bt.username, self.__bt.tool_id
        )
        try:
    ...
    def __delete_data(self, delete_branch_name, delete_file_path, file_sha):
        ...
        commit_message = self.__create_commit_message(
            self.DELETE, self.__bt.username, self.__bt.tool_id
        )
    ...
    def __create_pull_request(self, pr_type, username, tool_id, branch_name):
        ...
        try:
            pr = self.__repo.create_pull(
                title=pr_title, body=pr_body, head=branch_name, base=self.MASTER_BRANCH
            )
        except GithubException as e:
            if pr_type == self.CREATE:
                raise ToolCreationException(
                    "Pull request creation failed for tool CREATE operation.".format(
                        type=pr_type
                    )
                    + str(e),
                    branch=branch_name,
                ...
                raise ToolUpdateException(
                    "Pull request creation failed for tool UPDATE operation.".format(
                        type=pr_type
                    )
                    + str(e),
                    branch=branch_name,
                ...
                raise ToolDeleteException(
                    "Pull request creation failed for tool DELETE operation.".format(
                        type=pr_type
                    )
                    + str(e),
                    branch=branch_name,
    ...
    def __load_data(self, bt, delete=False):
        ...
        self.__branch = self.__get_branch_name(
            normalized_username, normalized_tool_id, delete
        )
    ...
    def create_tool(self, biotools_data):
        ...
        if self.__file_exists(new_file_path, new_branch_name):
            raise ToolCreationException(
                "The file already exists on branch: {b} at tool creation time.".format(
                    b=new_branch_name
                ),
                branch=new_branch_name,
                filepath=new_file_path,
    ...
    def update_tool(self, biotools_data):
        ...
        if not self.__branch_exists(update_branch_name) and not self.__file_exists(
            update_file_path, self.MASTER_BRANCH
        ):
            raise ToolUpdateException(
        ...
        if self.__branch_exists(update_branch_name) and not self.__file_exists(
            update_file_path, update_branch_name
        ):
            raise ToolUpdateException(
    ...
    def delete_tool(self, biotools_data):
        ...
        if not self.__file_exists(delete_file_path, self.MASTER_BRANCH):
            regular_non_delete_branch = self.__get_branch_name(
                self.__bt.username, self.__bt.tool_id
            )

            # We need to have an open pull request with the non delete branch
            #   and of course the actual branch, otherwise we fail
            if not self.__branch_exists(
                regular_non_delete_branch
            ) or not self.__pull_request_exists(regular_non_delete_branch):
                raise ToolDeleteException(
                    "The file does not exist on master branch: {b} or on any other user-related branch at tool delete time.".format(
                        b=self.MASTER_BRANCH
                    ),
            ...
            # the file is not on master and also not on a regular CREATE/UPDATE (non delete) branch
                ...
                raise ToolDeleteException(
                    "The file does not exist on any valid branches: {b} at tool delete time.".format(
                        b=self.MASTER_BRANCH
                    ),
        ...
        if file_not_on_master_branch_but_on_update_branch:
            self.__create_branch(
                delete_branch_name, from_branch=regular_non_delete_branch
            )
        ...
        if not self.__file_exists(delete_file_path, delete_branch_name):
            raise ToolDeleteException(
                "The file does not exist on branch: {b} at tool delete time.".format(
                    b=delete_branch_name
                ),
                branch=delete_branch_name,
                filepath=delete_file_path,
                sha=None,
        ...
        d = self.__delete_data(delete_branch_name, delete_file_path, file_sha)

        # file still exists on delete branch after delete then fail
        if self.__file_exists(delete_file_path, delete_branch_name):
            raise ToolDeleteException(
                "The file was not deleted on branch: {b} at tool delete time.".format(
                    b=delete_branch_name
                ),
                branch=delete_branch_name,
                filepath=delete_file_path,
    ...
    def create_issue(self, biotools_data=None, error_message=None):
        self.__repo.create_issue(
            title="Error in ecosystem from bio.tools; tool id: {}".format(
                biotools_data.tool_id
            ),
            body="""
                ...
                {}
            """.format(
                error_message, biotools_data.tool_json
            ),
        )
