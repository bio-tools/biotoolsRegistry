...
class BioToolsData(object):
    ...
    def __init__(self, username, tool_json_string, delete=False, tool_id=None):
        ...
        if not username:
            raise BioToolsException(
                username, tool_json_string, "No valid bio.tools username was provided."
            )
        ...
        if not tool_json_string:
            raise BioToolsException(
                username, tool_json_string, "Missing tool JSON data."
            )
        ...
        try:
            ...
            raise BioToolsException(
                username, tool_json_string, "Invalid tool JSON structure. " + str(v)
            )
    ...
    def validate_tool(self):
        ...
        if self.__tool.get(self.__ID_KEY) is None:
            raise BioToolsException(
                self.__username, self.__tool_json_string, "Missing tool id."
            )
