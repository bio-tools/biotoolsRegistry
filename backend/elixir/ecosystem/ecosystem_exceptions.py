...
class BioToolsException(Exception):
    ...
    def __str__(self):
        return "{m}; username={u}; tool_data={t}".format(
            m=self.__message, u=self.__username, t=json.dumps(self.__tool)
        )
...
class EcosystemException(Exception):
    ...
    def __init__(
        self, message, branch=None, filepath=None, sha=None, username=None, tool_id=None
    ):
        super(Exception, self).__init__()
        self.__message = message
    ...
    def __str__(self):
        if self.__message:
            return "Could not login to Github Ecosystem with username: {u} for repository: {r}; Reason: {m}".format(
                u=self.__username, r=self.__repo, m=self.__message
            )
