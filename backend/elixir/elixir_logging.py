...
from elixir.models import SearchQueryLog, SearchTermLog
...
class SearchLogger:
    ...
    def query_log_terms(self, data):
        ...
        exsiting_log_term = SearchTermLog.objects.filter(
            name=data["name"], term=data["term"]
        )
...
def search_log_json_data():
    ...
    for term in searchTerms:
        ...
        termData["type"] = (
            "query"
            if (termSerializer.data["name"] == "q")
            else termSerializer.data["name"]
        )
