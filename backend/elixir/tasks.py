...
def backup_stats_with_date(upperDateLimit=datetime.today()):
    ...
    statsData.operatingSystemAnnotationCount = statsData.data["totalAnnotationCount"][
        "operatingSystemAnnotationCount"
    ]
    statsData.codeAvailabilityAnnotationCount = statsData.data["totalAnnotationCount"][
        "codeAvailabilityAnnotationCount"
    ]
    statsData.operationAnnotationCount = statsData.data["totalAnnotationCount"][
        "operationAnnotationCount"
    ]
    statsData.descriptionAnnotationCount = statsData.data["totalAnnotationCount"][
        "descriptionAnnotationCount"
    ]
    statsData.downloadsAnnotationCount = statsData.data["totalAnnotationCount"][
        "downloadsAnnotationCount"
    ]
    statsData.dataFormatsAnnotationCount = statsData.data["totalAnnotationCount"][
        "dataFormatsAnnotationCount"
    ]
    statsData.accessibilityAnnotationCount = statsData.data["totalAnnotationCount"][
        "accessibilityAnnotationCount"
    ]
    statsData.toolTypeAnnotationCount = statsData.data["totalAnnotationCount"][
        "toolTypeAnnotationCount"
    ]
    statsData.documentationAnnotationCount = statsData.data["totalAnnotationCount"][
        "documentationAnnotationCount"
    ]
    statsData.inputOutputAnnotationCount = statsData.data["totalAnnotationCount"][
        "inputOutputAnnotationCount"
    ]
    statsData.communityAnnotationCount = statsData.data["totalAnnotationCount"][
    ...
    statsData.publicationAnnotationCount = statsData.data["totalAnnotationCount"][
        "publicationAnnotationCount"
    ]
    statsData.save()
    statsData.date = upperDateLimit
    statsData.save()
    return (
        "Statistics for " + str(statsData.totalEntries) + " entries saved successfully."
    )
...
def update_pmcid_only_publication_metadata():
    ...
    pubs = [
        p
        for p in Publication.objects.filter(pmcid__isnull=False).select_related(
            "resource"
        )
        if p.resource.visibility == 1
    ]
    ...
    for publication in pubs:
        print(
            (
                ...
                + str(currentPublicationIndex)
                + " out of "
                + str(publicationCount)
            )
        )
        update_publication(publication)
        currentPublicationIndex = currentPublicationIndex + 1
...
def notify_admins(identifier, subject, message):
    ...
    send_mail(
        subject, final_message, settings.DEFAULT_FROM_EMAIL, settings.ADMIN_EMAIL_LIST
    )
...
def create_ecosystem_tool(tool, user):
    ...
    tool_json = json.dumps(tool, sort_keys=True, indent=4, separators=(",", ": "))
...
def delete_ecosystem_tool(tool_id, user):
    ...
    tool_json = json.dumps(
        {"biotoolsID": tool_id}, sort_keys=True, indent=4, separators=(",", ": ")
    )
