...
class EDAMDataIssue(IssueBaseClass):
    ...
    def __init__(self, qset, user, term="Data"):
        ...
        super(EDAMDataIssue, self).__init__(
            _qset=qset,
            _type=IssueType.objects.get(
                type="ontology", attribute="data", field_value=None
            ),
            _user=user,
        )
    ...
    def detect(self, resource):
        ...
        self.issue_type = IssueType.objects.get(
            ...
            attribute="data",
            field_value=(
                obj["status"] if obj["status"] in ["obsolete", "not_found"] else None
            ),
        )
        ...
        for function in resource.function.all():
            for inp in function.input.all():
                if inp.data:
                    if (
                        inp.data.term == obj["data"]["text"]
                        or inp.data.uri == obj["data"]["data"]["uri"]
                    ):
            ...
            for out in function.output.all():
                if out.data:
                    if (
                        out.data.term == obj["data"]["text"]
                        or out.data.uri == obj["data"]["data"]["uri"]
                    ):
