...
class NoPublicationIssue(IssueBaseClass):
    ...
    def __init__(self, qset, user):
        super(NoPublicationIssue, self).__init__(
            _qset=qset,
            _type=IssueType.objects.get(
                type="missing", attribute="publication", field_name="id"
            ),
            _user=user,
        )
    ...
    def detect(self, resource):
        ...
        for publication in resource.publication.all():
            if (
                publication.pmid is None
                and publication.pmcid is None
                and publication.doi is None
            ):
