...
class DomainView(APIView):
	...
	def post(self, request, format=None):
		...
		x = re.match('^[a-zA-Z0-9-]{2,40}$', struct['domain'])
		if not x:
			return Response({"details": "Domain name can only contain alphanumeric characters and dashes, with length between 2 and 40 characters."}, status=status.HTTP_400_BAD_REQUEST)
