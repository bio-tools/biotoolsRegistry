...
class IssueView(APIView):
    ...
    def get(self, request, biotoolsID, issueId=None):
        if not biotoolsID:
            return Response(
                {"data": None, "text": "Missing id."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        issue_qs = Issue.objects.filter(resource_biotoolsID__iexact=biotoolsID)
        if issueId:
            issue_qs = issue_qs.filter(id=issueId)

        return Response(
            IssueSerializer(instance=issue_qs, many=True).data,
    ...
    def post(self, request, biotoolsID, issueId=None):
        ...
        if issueId:
            ...
            if not payload:
                return Response(
                    {"data": None, "text": "Missing payload."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            for x in ["comment", "issue_state"]:
                if x not in list(payload.keys()):
                    return Response(
                        {"data": None, "text": "Malformed payload."},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            try:
                ...
                return Response(
                    {"data": None, "text": "Cannot find issue with this ID."},
            ...
            if i.issue_type.id != i_type.id:
                return Response(
                    {
                        "data": None,
                        "text": "Cannot make changes to the non-custom issues.",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                IssueSerializer(instance=i, many=False).data, status=status.HTTP_200_OK
            )
