...
class DownloadSerializer(serializers.ModelSerializer):
    url = serializers.CharField(
        allow_blank=False,
        validators=[IsURLFTPValidator, is_blacklisted_url_validator],
        required=True,
    )
    type = serializers.CharField(
        allow_blank=True, max_length=300, min_length=1, required=True
    )
    ...
    version = serializers.CharField(
        allow_blank=True, max_length=100, min_length=1, required=False
    )
