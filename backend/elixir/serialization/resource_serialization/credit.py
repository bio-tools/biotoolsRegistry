...
class CreditSerializer(serializers.ModelSerializer):
    ...
    url = serializers.CharField(
        allow_blank=False,
        validators=[IsURLValidator, is_blacklisted_url_validator],
        required=False,
    )
    email = serializers.CharField(
        allow_blank=False,
        max_length=300,
        validators=[IsStringTypeValidator, IsEmailValidator],
        required=False,
    )
    orcidid = serializers.CharField(
        allow_blank=False, validators=[IsStringTypeValidator], required=False
    )
    gridid = serializers.CharField(
        allow_blank=False, validators=[IsStringTypeValidator], required=False
    )
    rorid = serializers.CharField(
        allow_blank=False, validators=[IsStringTypeValidator], required=False
    )
    fundrefid = serializers.CharField(
        allow_blank=False, validators=[IsStringTypeValidator], required=False
    )
    ...
    def validate_orcidid(self, attrs):
        ...
        if not p.search(attrs):
            raise serializers.ValidationError(
                "This field can only contain a valid ORCID ID"
            )
        return attrs
    ...
    def validate(self, data):
        if (
            ...
            and data.get("url") is None
            and data.get("email") is None
        ):
            raise serializers.ValidationError(
                "At least one of credit name, credit email or credit URL is mandatory."
            )
        return data
    ...
    def validate_gridid(self, attrs):
        ...
        if not p.search(attrs):
            raise serializers.ValidationError(
                "This field can only contain a valid GRID ID"
            )
        return attrs
    ...
    def validate_rorid(self, attrs):
        ...
        if not p.search(attrs):
            raise serializers.ValidationError(
                "This field can only contain a valid ROR ID"
            )
        return attrs

    def validate_fundrefid(self, attrs):
        p = re.compile(
            r"^10\.13039\/[-\.\[\]<>_;\(\)\/:a-zA-Z0-9]+$", re.IGNORECASE | re.UNICODE
        )
        if not p.search(attrs):
            raise serializers.ValidationError(
                "This field can only contain a valid FundRef ID"
            )
        return attrs
