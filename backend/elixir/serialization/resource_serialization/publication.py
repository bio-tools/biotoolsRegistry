...
class PublicationSerializer(serializers.ModelSerializer):
    pmcid = serializers.CharField(
        allow_blank=False, validators=[IsPMCIDValidator], required=False
    )
    pmid = serializers.CharField(
        allow_blank=False, validators=[IsPMIDValidator], required=False
    )
    doi = serializers.CharField(
        allow_blank=False, validators=[IsDOIValidator], required=False
    )
    ...
    version = serializers.CharField(
        allow_blank=False, max_length=100, min_length=1, required=False
    )
    ...
    def validate(self, data):
        if not data.get("doi") and not data.get("pmid") and not data.get("pmcid"):
            raise serializers.ValidationError(
                "Publication requires at least one of DOI, PMID, PMCID"
            )
