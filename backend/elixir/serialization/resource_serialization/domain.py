...
from elixir.models import Domain, DomainCollection, DomainResource, DomainTag, Resource
...
class DomainSerializer(serializers.ModelSerializer):
    domain = serializers.CharField(
        ...
        validators=[
            ...
            UniqueValidator(
                queryset=Domain.objects.filter(visibility=1),
                message="A domain with this name already exists. Choose a different domain name",
            ),
        ],
    )
    ...
    resources = DomainResourceSerializer(
        source="resource", many=True, required=False, allow_empty=True, allow_null=False
    )
    tag = DomainTagSerializer(
        many=True, required=False, allow_empty=True, allow_null=False
    )
    collection = DomainCollectionSerializer(
        many=True, required=False, allow_empty=True, allow_null=False
    )
    ...
    def validate_domain(self, domain_name):
        if (
            self.context["request_type"] in ["POST", "PUT", "DELETE"]
            and domain_name.strip().lower() == "all"
        ):
        ...
        if not p.search(domain_name):
            raise serializers.ValidationError(
                "Domain name can only contain alphanumeric characters and dashes, with length between 2 and 40 characters."
            )
    ...
    def create(self, validated_data):
        ...
        resources_list = (
            pop(validated_data, "resource")
            if "resource" in list(validated_data.keys())
            else []
        )
        seen = set()
        resources_list = [
            seen.add(obj["biotoolsID"]) or obj
            for obj in resources_list
            if obj["biotoolsID"] not in seen
        ]
