...
class TestDocsDownloadSource(BaseTestObject):
	...
	def test_docsDownloadSource_correct(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_correct', format='json')
	...
	def test_docsDownloadSource_empty_none(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_empty_none')
	...
	def test_docsDownloadSource_empty_zero_length(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_empty_zero_length')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_docsDownloadSource_300_length(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_300_length', format='json')
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_docsDownloadSource_301_length(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_301_length')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_docsDownloadSource_non_ascii(self):
		...
		sent_resource['docs'] = {
			'docsDownloadSource': u'http://ąęćżźń£ØΔ♥†.com'
		}

		response = self.client.post('/tool/', sent_resource, format='json')
		self.assertEqual(response.status_code, status.HTTP_201_CREATED)

		response = self.client.get('/tool/test_docsDownloadSource_non_ascii', format='json')
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_docsDownloadSource_incorrect(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_incorrect')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_docsDownloadSource_array(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_array')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_docsDownloadSource_dict(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_dict')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_docsDownloadSource_number(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_number')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_docsDownloadSource_case_sensitive(self):
		...
		response = self.client.get('/tool/test_docsDownloadSource_case_sensitive', format='json')
		self.assertEqual(response.status_code, status.HTTP_200_OK)
