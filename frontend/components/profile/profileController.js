angular.module('elixir_front').controller('ProfileController', [
    '$scope',
    '$state',
    'djangoAuth',
    'DisownToolService',
    'User',
    '$uibModal',
    function ($scope, $state, djangoAuth, DisownToolService, User, $uibModal) {
        $scope.profile = {};
        $scope.loading = true;
        $scope.User = User;
        djangoAuth.profile().then(
            function (response) {
        ...
        $scope.disownEntry = function (entry) {
            ...
            DisownToolService.disown(
                { id: entry.id, version: entry.versionId },
                function (response) {
