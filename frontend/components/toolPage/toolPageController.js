    ...
    .controller('ToolPageController', [
        ...
        function (
            ...
            $scope.functionNameClicked = function (functionName) {
                $state.go('search', {
                    operationID: quoteQueryStringValue(stripEdam(functionName.uri)),
                });
            };
            ...
            $scope.inputNameClicked = function (inputName) {
                ...
                $state.go('search', {
                    inputDataTypeID: quoteQueryStringValue(stripEdam(inputName.data.uri)),
                });
            };
            ...
            $scope.inputFormatNameClicked = function (formatName) {
                ...
                $state.go('search', {
                    inputDataFormatID: quoteQueryStringValue(stripEdam(formatName.uri)),
                });
            };

            $scope.outputFormatNameClicked = function (formatName) {
                $state.go('search', {
                    outputDataFormatID: quoteQueryStringValue(stripEdam(formatName.uri)),
                });
            };

            $scope.outputNameClicked = function (outputName) {
                $state.go('search', {
                    outputDataTypeID: quoteQueryStringValue(stripEdam(outputName.data.uri)),
                });
            };
            ...
            $scope.requestEditing = function (resourceId) {
                $scope.editingRequestedSuccess = null;
                ResourceRequestProvider.requestEditingRights(resourceId).then(
                    function successCallback(response) {
            ...
            $scope.software.$promise.then(
                function (data) {
                    // console.log('Success: '+JSON.stringify(data));
                    // console.log("calling API "+"/api/"+data.biotoolsID+"?format=jsonld");
                    $http
                        .get('/api/' + data.biotoolsID + '?format=jsonld')
    ...
    .directive('toolPageLinkCallout', function () {
        return {
            ...
            template: function (elem, attr) {
                return (
                    '<div class="bs-callout-sm bs-callout-primary" style="border-left-color: ' +
                    attr.color +
                    ';">' +
                    '<div class="tool-page-callout-header" style="color: black;">' +
                    '<a target="_blank" href="' +
                    attr.url +
                    '" tooltips tooltip-side="top" tooltip-content="' +
                    attr.url +
                    '">' +
                    attr.name +
                    ' › </a>' +
                    '<i ng-show="' +
                    attr.toshow +
                    '" class="fa fa-question-circle" aria-hidden="true" style="font-size: 100%; margin-left: 0.5em; color: ' +
                    attr.color +
                    ';" tooltips tooltip-side="top" tooltip-content="' +
                    attr.comment +
                    '"></i>' +
        ...
        return {
            ...
            template: function (elem, attr) {
                ...
                callout +=
                    '<div ng-show="' +
                    attr.name +
                    '" class="tool-page-callout-header" style="color: black;">{{::' +
                    attr.name +
                    '}}</div>';
                ...
                callout +=
                    ...
                    ".replace('@', ' at ') }}";
                callout +=
                    '<span ng-show="' + attr.url + '" style="color: #CCCCCC;"> | </span></span>';
                callout +=
                    '<span ng-show="' +
                    attr.url +
                    '"><a href="{{::' +
        ...
        return {
            ...
            template: function (elem, attr) {
                ...
                callout +=
                    '<div ng-show="' +
                    attr.name +
                    '" class="tool-page-callout-header" style="color: black;"><a href="" ng-click="creditNameClicked(credit.name)">{{' +
                    attr.name +
                    '}}</a>';
                callout +=
                    '<i ng-show="' +
                ...
                callout +=
                    ...
                    " != 'Person'\">";
                callout +=
                    '<span ng-show="' + attr.typeentity + '" style="color: #CCCCCC;"> | </span>';
                callout +=
                    '<span ng-show="' + attr.typeentity + '">{{' + attr.typeentity + '}}</span>';
                callout += '</div>';
                callout += '<div class="tool-page-callout-text">';
                callout +=
                    '<span ng-show="' +
                ...
                callout +=
                    ...
                    '"><a href="{{' +
                    attr.orcidid +
                    '}}" target="_blank">ORCID ›</a></span>';
                callout +=
                    '<span ng-show="' + attr.gridid + '" style="color: #CCCCCC;"> | </span></span>';
                callout +=
                    '<span ng-show="' +
                    attr.gridid +
                    '"><a href="https://www.grid.ac/institutes/{{' +
                    attr.gridid +
                    '}}" target="_blank">gridid ›</a></span>';
                callout +=
                    '<span ng-show="' + attr.rorid + '" style="color: #CCCCCC;"> | </span></span>';
                callout +=
                    '<span ng-show="' +
                    attr.rorid +
                    '"><a href="https://ror.org/{{' +
                    attr.rorid +
                    '}}" target="_blank">rorid ›</a></span>';
                callout +=
                    '<span ng-show="' +
                    attr.fundrefid +
                    '" style="color: #CCCCCC;"> | </span></span>';
                callout +=
                    ...
                    attr.fundrefid +
                    '}}" target="_blank">fundrefid ›</a></span>';
                callout += '</div>';
                callout += '</div>';
                return callout;
            },
