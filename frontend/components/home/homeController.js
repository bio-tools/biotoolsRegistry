angular.module('elixir_front').controller('HomeController', [
    '$scope',
    '$state',
    '$stateParams',
...
angular.module('elixir_front').directive('homeSearch', [
    'Query',
    'ToolList',
    'Highlighting',
    'Attribute',
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.topicNameClicked = function (topic) {
                    $state.transitionTo(
                        'search',
                        { topic: quoteQueryStringValue(topic), page: 1, sort: 'score' },
                        {
                            reload: true,
                            inherit: false,
                            notify: true,
                ...
                scope.operationNameClicked = function (operation) {
                    $state.transitionTo(
                        'search',
                        { operation: quoteQueryStringValue(operation), page: 1, sort: 'score' },
                        {
                            reload: true,
                            inherit: false,
                            notify: true,
                ...
                scope.getSuggestionsFor = function (term) {
                    ...
                    if (!scope.usedTermsLoaded[term]) {
                        ...
                        scope.usedTerms[term] = getUsedTerms(
                            term === 'everything' ? 'all' : term,
                            params
                        );
                ...
                scope.doHomeSearch = function () {
                    ...
                    if (
                        no_params > 0 &&
                        scope.Query.current[no_params - 1].text == scope.lastParam &&
                        (params != null) & (params.page != undefined)
                    ) {
