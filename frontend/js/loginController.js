// Controllers
angular.module('elixir_front.controllers').controller('LoginMangerController', [
    '$scope',
    '$uibModal',
    '$rootScope',
    '$state',
    function ($scope, $uibModal, $rootScope, $state) {
        var vm = this;
...
function LoginModalController($uibModalInstance, djangoAuth, $rootScope, $state) {
    ...
    vm.loginButtonPressed = function (username, password) {
        ...
        if (!username && !password) {
            ...
            djangoAuth.login(username, password).then(
                function (response) {
