// Controllers
angular.module('elixir_front.controllers').controller('RequestsController', [
    '$scope',
    'ResourceRequestProvider',
    'User',
    function ($scope, ResourceRequestProvider, User) {
        var vm = this;
        $scope.requests = {};
        $scope.received = [];
        $scope.sent = [];
        ...
        vm.loadRequests = function ($item, $model, $label) {
            $scope.loading = true;
            ResourceRequestProvider.getRequests().then(
                ...
                function errorCallback(response) {
                    ...
                    $scope.error.message =
                        'There was an error communicating with the server. Try refreshing this website.';
                    $scope.loading = false;
                }
            );
        };
        ...
        vm.requestsAffectedByAcceptingRequest = function (request) {
            ...
            if (request.type == 'ownership' && request.username != User.current.username) {
                ...
                for (var index = 0; index < $scope.requests.received.length; ++index) {
                    ...
                    if (
                        checkRequest.completed == false &&
                        checkRequest.requestId != request.requestId &&
                        checkRequest.resourceId == request.resourceId
                    ) {
                ...
                if (
                    confirm(
                        'Are you sure you want to accept this request? ' +
                            ...
                            ' of the exisitng requests will be sent to the new owner.'
                    )
                ) {
                    vm.processAcceptRequest(resourceIndex, request, affectedRequests);
                }
        ...
        vm.processAcceptRequest = function (index, request, affectedRequests) {
            ...
            ResourceRequestProvider.acceptRequest(request.requestId).then(
                function successCallback(response) {
                    ...
                    $scope.requests.received = _.difference(
                        $scope.requests.received,
                        affectedRequests
                    );
                    ...
                    User.current.requests_count =
                        User.current.requests_count - 1 - affectedRequests.length;
                },
                function errorCallback(response) {
                    vm.loadRequests();
                }
        ...
        $scope.declineRequest = function (request) {
            ...
            request.processing = false;
            $scope.requests.received[resourceIndex].processing = true;
            ResourceRequestProvider.declineRequest(request.requestId).then(
                function successCallback(response) {
                    $scope.requests.received[resourceIndex].processing = false;
                    $scope.requests.received[resourceIndex].completed = true;
...
elixir_front.factory('ResourceRequestProvider', [
    '$http',
    function ($http) {
        return {
