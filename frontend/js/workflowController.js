// Controllers
angular.module('elixir_front.controllers').controller('WorkflowController', [
    '$scope',
    '$stateParams',
    '$uibModal',
    'WorkflowProvider',
    function ($scope, $stateParams, $uibModal, WorkflowProvider) {
        var vm = this;
        $scope.loading = true;
        ...
        vm.loadRequests = function ($item, $model, $label) {
            $scope.loading = true;
            WorkflowProvider.getWorkflows().then(
            ...
            if (searchString == undefined || searchString == '') {
                ...
                $scope.filteredWorkflows = $scope.workflows.filter(function (x) {
                    var foundAnnotations = x.annotations.filter(function (annotation) {
                        ...
                        if (
                            annotation.title.match(regularExpression) ||
                            annotation.title.match(regularExpression) ||
                            annotation.edam_term.match(regularExpression)
                        ) {
                            return true;
                        }
                        return false;
...
function WorkflowModalCtrl($uibModalInstance, workflow) {
    ...
    vm.mapDimensions = function (annotation) {
        return (
            vm.mapXDimension(annotation.startX) +
            ',' +
            vm.mapYDimension(annotation.startY) +
            ',' +
            vm.mapXDimension(annotation.endX) +
            ',' +
            vm.mapYDimension(annotation.endY)
        );
    };
...
elixir_front.factory('WorkflowProvider', [
    '$http',
    function ($http) {
        return {
