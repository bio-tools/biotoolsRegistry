...
angular
    .module('elixir_front.directives', [])
    .directive('toolList', [
        'ToolList',
        'Highlighting',
        function (ToolList, Highlighting) {
            return {
                restrict: 'A',
                ...
                controller: [
                    ...
                    function ($scope, $element, $interval) {
                        ...
                        function renderStyles() {
                            if (
                                $element.height() >= $scope.showMoreHeight &&
                                $scope.expanded === false
                            ) {
    ...
    .directive('toolListTable', [
        'ToolList',
        'ToolTableDataSource',
        'Highlighting',
        function (ToolList, ToolTableDataSource, Highlighting) {
            return {
                restrict: 'A',
    ...
    .directive('toolSorter', [
        ...
        'Query',
        '$rootScope',
        function (ToolList, $state, $stateParams, $timeout, ToolSorter, Query, $rootScope) {
            return {
                restrict: 'A',
                ...
                link: function (scope, element, attrs) {
                    ...
                    scope.addToURL = function () {
                        ...
                        if (
                            ToolSorter.sortBy.attrName == 'lastUpdate' &&
                    ...
                    if ($stateParams['sort'] != null) {
                        ...
                        ToolSorter.sortBy =
                            ToolSorter.list[
                                _.findIndex(ToolSorter.list, { attrName: $stateParams['sort'] }) ||
                                    0
                            ];
                    }
    ...
    .directive('displayModeSelector', [
        'ToolList',
        '$state',
        '$stateParams',
        '$timeout',
        'DisplayModeSelector',
        function (ToolList, $state, $stateParams, $timeout, DisplayModeSelector) {
            return {
                restrict: 'A',
                ...
                link: function (scope, element, attrs) {
                    ...
                    scope.pageChanged = function () {
                        ...
                        $timeout(function () {
                            ...
                            params['page'] =
                                ToolPaginator.currentPage != 1 ? ToolPaginator.currentPage : null;
                            $state.transitionTo('search', params, { notify: false });
                            ToolList.refresh();
                        });
                    };
                    ...
                    scope.handlePageInput = function (event) {
                        if (event.key === 'Enter') {
                            ...
                            if (
                                scope.inputPage >= 1 &&
                                scope.inputPage <=
                                    Math.ceil(ToolList.count / ToolPaginator.pageSize)
                            ) {
                    ...
                    scope.$watch('ToolPaginator.currentPage', function (newVal, oldVal) {
                        if (initializing) {
                            if (newVal != oldVal && newVal == 1) {
                                ToolPaginator.currentPage = oldVal;
                            }
                            $timeout(function () {
                                initializing = false;
                            });
            ...
            compile: function CompilingFunction($templateElement, $templateAttributes) {
                return function LinkingFunction($scope, $element, $attrs) {
                    ...
                    if (/^\d{7,8}$/.test($scope.publication)) {
                        html =
                            '<span>PUBMED:</span> <a href="http://www.ncbi.nlm.nih.gov/pubmed/' +
                            $scope.publication +
                            '">' +
                            $scope.publication +
                            '</a>';
                        ...
                        html =
                            '<span>PMC:</span> <a href="http://www.ncbi.nlm.nih.gov/pmc/' +
                            $scope.publication +
                            '">' +
                            $scope.publication +
                            '</a>';
                        ...
                        html =
                            '<span>DOI:</span> <a href="https://dx.doi.org/' +
                            publication +
                            '">' +
                            publication +
                            '</a>';
                ...
                return function LinkingFunction($scope, $element, $attrs) {
                    ...
                    if (/^\d{7,8}$/.test($scope.publication)) {
                        html =
                            ...
                            $scope.publication +
                            '"><i class="fa fa-external-link" aria-hidden="true" uib-tooltip="PUBMED" tooltip-append-to-body="true"></i></a>';
                        // PMC
                    } else if (/^PMC\d{6,8}$/.test($scope.publication)) {
                        html =
                            '<a href="http://www.ncbi.nlm.nih.gov/pmc/' +
                            $scope.publication +
                            '"><i class="fa fa-external-link" aria-hidden="true" uib-tooltip="PMC" tooltip-append-to-body="true"></i></a>';
                        // DOI
                    } else if (/^(doi:)?\d{2}.\d{4}/.test($scope.publication)) {
                        var publication = $scope.publication.replace('doi:', '');
                        html =
                            ...
                            publication +
                            '"><i class="fa fa-external-link" aria-hidden="true" uib-tooltip="DOI" tooltip-append-to-body="true"></i></a>';
                    }
                    var e = $compile(html)($scope);
                    $element.replaceWith(e);
                };
    ...
    .directive('resourceNotFoundDialog', function () {
        return {
            ...
            replace: 'true',
            template:
                '<div class="col-md-12 info-dialog-background info-dialog info-dialog-text">We\'re sorry but we couldn\'t find what you were looking for. You can try searching for the resource you need <a ui-sref="search()">here</a>.</div>',
        };
    })
    ...
    .directive('errorDialog', function () {
        return {
            ...
            template: function (elem, attr) {
                return (
                    '<div class="col-md-offset-2 col-md-8 bs-callout bs-callout-danger" style="margin-top: 80px;"><h4><i class="fa fa-exclamation-circle" aria-hidden="true"></i> Error {{' +
                    attr.errorcode +
                    '}}</h4>{{' +
                    attr.errormessage +
                    '}}</div>'
    ...
    .directive('validateEditResourceField', function () {
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                var dataPath = attrs.ngModel
                    .replace('software.', '')
                    .replace('$parent.$index', attrs.parentIndex)
                    .replace('$index', attrs.index)
                    .replace('[', '.')
                    .replace(']', '')
                    .split('.');
    ...
    .directive('smallSpinner', function () {
        return {
            ...
            replace: 'true',
            template:
                '<div style="text-align:center; color: #7f7f7f; font-size: 12px;"><i class="fa fa-circle-o-notch fa-spin"></i> Processing...</div>',
        };
    })
    .directive('largeSpinner', function () {
        return {
            ...
            replace: 'true',
            template:
                '<div style="text-align:center; color: #cccccc; font-size: 60px;"><i class="fa fa-circle-o-notch fa-spin"></i></div>',
        };
    });
