...
.directive('toolListDashboard', ['ToolList', 'Highlighting', 'UsedTerms', '$state', '$stateParams', function(ToolList, Highlighting, UsedTerms, $state, $stateParams) {
    return {
        restrict: 'A',
        templateUrl: 'partials/directives/toolListDashboard.html',
        link: function(scope, element, attrs) {
            // reference the ToolList and Highlighting in the scope
            scope.ToolList = ToolList;
            scope.Highlighting = Highlighting;

            // sections displayed in the filter sidebar; 'ontology' is the
            // /api/used-terms/<ontology> endpoint name, 'param' is the search
            // state parameter used to apply the filter
            var filterSections = [
                {param: 'topic', ontology: 'topic', label: 'Topic', searchable: true},
                {param: 'toolType', ontology: 'toolType', label: 'Tool type'},
                {param: 'operation', ontology: 'operation', label: 'Operation', searchable: true},
                {param: 'inputData', ontology: 'inputData', label: 'Input data', searchable: true},
            ...
            // how many values to show per section before "show more"
            scope.visibleOptions = 5;

            // which sections are expanded to show all values
            scope.expandedSections = {};

            // which filter dropdowns are open; sections with an active value
            // start open so the current filter is visible
            scope.openSections = {};
            _.forEach(filterSections, function(section) {
                if (unquote($stateParams[section.param])) {
                    scope.openSections[section.param] = true;
                }
            });

            // open/close a filter dropdown; a section starts open if a value
            // for it is already applied so the active filter stays visible
            scope.toggleSection = function(section) {
                scope.openSections[section.param] = !scope.openSections[section.param];
            };

            // unquote a state param value for comparison ('"MIT"' -> 'MIT')
            function unquote(value) {
                return (value || '').replace(/^"+|"+$/g, '');
            }

            // the currently applied value for a section, or null
            scope.activeValue = function(section) {
                var value = $stateParams[section.param];
                return value ? unquote(value) : null;
            };

            // build params for the used-terms request: current params minus
            // pagination/sorting/domain and minus the section's own field, so
            // that a field's active filter does not hide its other values
            function usedTermsParams(section) {
                var params = _.clone($stateParams);
                params['page'] = null;
                params['sort'] = null;
                params['ord'] = null;
                params[section.param] = null;
                params['usedTermName'] = section.ontology;
                return params;
            }

            // fetch the option lists for all sections
            scope.sectionOptions = {};
            _.forEach(filterSections, function(section) {
                if (section.staticOptions) {
                    scope.sectionOptions[section.param] = section.staticOptions;
                    return;
                }
                scope.sectionOptions[section.param] = [];
                UsedTerms.get(usedTermsParams(section), function(response) {
                    scope.sectionOptions[section.param] = response.data || [];
                });
            });

            // filter text for searchable sections (topic/operation/inputData/outputData)
            scope.optionFilter = {};

            // local text filter for a section's options
            scope.filteredOptions = function(section) {
                var options = scope.sectionOptions[section.param] || [];
                var text = scope.optionFilter[section.param];
                if (!text) {
                    return options;
                }
                var lower = text.toLowerCase();
                return _.filter(options, function(option) {
                    return option.toLowerCase().indexOf(lower) !== -1;
                });
            };

            // apply or clear a filter: single-select per field, quoted for
            // exact matching (same pattern as topicNameClicked)
            scope.filterClicked = function(section, option) {
                var params = _.clone($stateParams);
                params['page'] = 1;
                if (scope.activeValue(section) === option) {
                    // clicking the active value removes the filter
                    params[section.param] = null;
                } else {
                    params[section.param] = '"' + option + '"';
                }
                $state.transitionTo('search', params, {reload: true, notify: true});
            };
        }
    }
}])
