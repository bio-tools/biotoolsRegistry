...
angular
    ...
    .controller('SubdomainAdminController', [
        ...
        'User',
        function ($scope, $stateParams, DomainConnection, DomainDetailConnection, User) {
            var vm = this;
            $scope.User = User;
            ...
            $scope.deleteSubdomain = function (name) {
                if (confirm('Are you sure you want to remove this subdomain?')) {
                    ...
                    var deleteResponse = DomainDetailConnection.delete(
                        { domain: name },
                        function (data) {
                            $scope.subdomains.splice(
                                $scope.subdomains.findIndex(function (d) {
            ...
            vm.loadSubdomains = function () {
                $scope.isLoadingSubdomains = true;
                $scope.subdomains = [];
                var domainResponse = DomainConnection.query(
                    {},
                    function (data) {
    ...
    .controller('DomainListController', [
        ...
        'DomainDetailConnection',
        '$state',
        function ($scope, DomainDetailConnection, $state) {
            var vm = this;
            $scope.domains = [];
        ...
        function (
            ...
            $scope.toolInSubdomain = function (toolID, versionID) {
                for (var index in $scope.subdomain.resources) {
                    ...
                    if (currentTool['biotoolsID'] == toolID) {
                        // && currentTool['versionId'] == versionID) {
                        return true;
                    }
                }
                return false;
            ...
            $scope.addToSubdomain = function (index) {
                ...
                var selectedDict = {
                    biotoolsID: selectedTool.biotoolsID,
                    versionId: selectedTool.versionId,
                };
                $scope.subdomain.toolList.push(selectedTool);
                $scope.subdomain.resources.push(selectedDict);
                $scope.subdomain.totalPages =
                    ($scope.subdomain.toolList.length / $scope.toolsPerPageCount) * 10;
            };
            ...
            $scope.addAllToSubdomain = function () {
                for (var tool in $scope.search.searchResults) {
                    ...
                    if (
                        selectedTool.biotoolsID &&
                        $scope.toolInSubdomain(selectedTool.biotoolsID, selectedTool.versionId) ==
                            false
                    ) {
                        $scope.subdomain.toolList.push(selectedTool);
                        $scope.subdomain.resources.push({
                            biotoolsID: selectedTool.biotoolsID,
                            versionId: selectedTool.versionId,
                        });
                    }
            ...
            $scope.saveSubdomain = function () {
                ...
                $scope.errors.general = '';
                $scope.response.general = '';
                var createResponse = DomainConnection.create(
                    vm.subdomainQuery(),
                    function (data) {
                        ...
                        $scope.response.general =
                            "Domain '" + $scope.subdomain.domain + "' was created.";
                        $scope.updating = false;
                    },
                    function (error) {
            ...
            vm.fetchSubdomain = function () {
                $scope.errors.general = '';
                $scope.response.general = '';
                var updateResponse = DomainDetailConnection.query(
                    ...
                    function (error) {
                        // Handle error
                        $scope.errors.loading =
                            'Data failed to load. Please try to refresh this website. ' +
                            error.data.details;
                    }
                );
            };
            ...
            $scope.updateSubdomain = function () {
                ...
                $scope.errors.general = '';
                $scope.response.general = '';
                var updateResponse = DomainDetailConnection.update(
                    vm.subdomainQuery(),
                    function (data) {
                        $scope.response.general =
                            "Domain '" +
                            $scope.subdomain.domain +
                            "' was updated with " +
                            $scope.subdomain.resources.length +
                            ' entries.';
                        $scope.updating = false;
                    },
                    function (error) {
            ...
            $scope.makeSubdomainURLSafe = function (value) {
                ...
                if (value != id) {
                    $scope.errors.domain =
                        "Entered value '" +
                        value +
                        "' has been modified. Only alphanumeric characters and dashes can be used for the subdomain name.";
                }
            ...
            vm.performSearch = function (query) {
                ...
                var response = ToolListOverviewConnection.query(query, function () {
                    ...
                    $scope.search.loadingSearchResults = false;
                    $scope.search.totalPages =
                        ($scope.search.searchResults.length / $scope.toolsPerPageCount) * 10;
...
angular.module('elixir_front').factory('UserSuggestionsProvider', [
    '$http',
    function ($http) {
        return {
